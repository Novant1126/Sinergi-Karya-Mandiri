    ...
    if ($stmt->execute()) {
        ...
        if (!empty($foto_instalasi['name'][0])) {
            for ($j = 0; $j < count($foto_instalasi['name']); $j++) {
                ...
                // Proses upload foto
                $foto_name = basename($foto_instalasi['name'][$j]);  // Ambil hanya nama file
                $foto_tmp = $foto_instalasi['tmp_name'][$j];
                $foto_path = 'uploads/foto_instalasi/' . $foto_name;  // Gabungkan dengan folder penyimpanan

                // Pindahkan foto ke folder 'uploads'
                if (move_uploaded_file($foto_tmp, $foto_path)) {
                    // Insert data foto instalasi ke tabel `instalations` dengan menyimpan nama file saja
                    $stmt = $conn->prepare("INSERT INTO instalations (id_lift, foto_instalasi, nama_instalasi, deskripsi) VALUES (?, ?, ?, ?)");
                    $stmt->bind_param("isss", $id_lift, $foto_name, $nama, $deskripsi);  // Simpan hanya nama file
                    $stmt->execute();
                }
            }
        }
