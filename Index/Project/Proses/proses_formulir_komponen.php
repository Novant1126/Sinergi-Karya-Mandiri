...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $conn = new mysqli('localhost', 'root', '', 'sinergi');
    if ($conn->connect_error) {
        die(json_encode(['status' => 'error', 'message' => 'Koneksi database gagal: ' . $conn->connect_error]));
    }

    $uploadDir = 'uploads/foto_bukti/';
    if (!is_dir($uploadDir)) {
    ...
    foreach ($data as $index => $row) {
        ...
        if (isset($_FILES['data']['name'][$index]['foto']) && $_FILES['data']['error'][$index]['foto'] === UPLOAD_ERR_OK) {
            $fotoName = uniqid() . '_' . basename($_FILES['data']['name'][$index]['foto']); // Tambahkan uniqid untuk nama unik
            $fotoPath = $uploadDir . $fotoName; // Path lengkap untuk menyimpan file
            ...
            $fotoPath = $fotoName; // Simpan hanya nama file di variabel
        }

        // Simpan data ke database
        $query = $conn->prepare(
            "INSERT INTO audit_komponen (id_gedung, id_tower, id_lift, id_komponen, id_temuan, id_solusi, prioritas, keterangan, foto_bukti) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
        );
