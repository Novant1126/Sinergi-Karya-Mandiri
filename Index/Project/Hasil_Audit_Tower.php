...
<body class="bg-light">
  ...
  <div class="container-fluid p-5">
    ...
    <div class="btn-group btn-sm mb-3" role="group" aria-label="Export Buttons">
      <!-- Tombol Export Excel -->
      <a href="../../Export/export-excel/export.php?id_gedung=<?= $id_gedung ?>" target="_blank">
      ...
      <a href="../../Export/Export-Word/generate_word.php?id_gedung=<?= $id_gedung ?>">
          <button id="exportWord" class="btn btn-primary btn-sm" title="Export to Word">
              <i class="fas fa-file-word"></i>
          </button>
      </a>
      
      </button>
      <a href="../../Export/Export-PPTX/generate_pptx.php?id_gedung=<?= $id_gedung ?>">
          <button id="export-btn" class="btn btn-warning btn-sm" title="Export to PPTX">
      ...
      <!-- <script>
        $(document).ready(function () {
            $('#export-btn').click(function () {
                $.ajax({
                    url: 'generate_pptx.php',
                    method: 'POST',
                    success: function (response) {
                        alert('File PPTX berhasil dibuat!');
                        window.location.href = 'PPTX/Audit-1.pptx'; // Redirect untuk download file
                    },
                    error: function () {
                        alert('Gagal membuat file PPTX.');
                    }
                });
            });
        });
