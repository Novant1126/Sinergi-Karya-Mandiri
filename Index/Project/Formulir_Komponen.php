...
<body class="bg-gray-100">
  ...
  <div class="table-responsive mt-4">
    <table class="table table-bordered table-striped table-hover" id="temporaryTable">
      ...
      <tbody>
        <!-- Data Tabel Ditambahkan Secara Dinamis -->
      </tbody>
    </table>
  </div>
  ...
  const rowIndex = dataAll.length - 1;
  const row = `
      <tr data-index="${rowIndex}">
          <td>${komponenText}<input type="hidden" name="id_komponen" value="${komponen}"></td>
          <td contenteditable="true" class="editable" data-field="temuanText">${temuanText}</td>
          <td contenteditable="true" class="editable" data-field="solusiText">${solusiText}</td>
          <td contenteditable="true" class="editable" data-field="prioritasText">${prioritasText}</td>
          <td contenteditable="true" class="editable" data-field="keterangan">${keterangan}</td>
          <td>${fotoFile ? fotoFile.name : "Tidak ada foto"}</td>
          <td><span class="text-danger deleteRow">Hapus</span></td>
      </tr>`;
...
tableBody.addEventListener("click", function (event) {
  ...
  if (event.target && event.target.classList.contains("deleteRow")) {
    ...
    if (row) {
      ...
      dataAll.splice(rowIndex, 1); // Hapus data dari array
    }
  }
});

// Perbarui data saat kolom diedit
tableBody.addEventListener("input", function (event) {
  ...
  if (cell) {
    ...
    if (dataAll[rowIndex] && field) {
      dataAll[rowIndex][field] = newValue; // Perbarui data di array
    }
  }
});
...
.then((result) => {
    alert(result.message); // Tampilkan notifikasi ke pengguna
    if (result.status === "success") {
        // Redirect ke halaman lain setelah operasi berhasil
        window.location.href = "../../Dashboad.php";
    } else {
        // Tetap di halaman ini jika ada error
        console.warn("Operasi gagal:", result.message);
    }
})
// .catch((error) => {
//     console.error(error);
