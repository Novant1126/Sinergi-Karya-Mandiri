...
if ($data) {
    ...
    foreach ($data as $row) {
        ...
        if (!isset($uniqueData[$uniqueKey])) {
            ...
            if (!empty($fotoinstalasi) && file_exists($imagePathinstalasi)) {
                // Gambar di slide
...
// Array untuk menyimpan data tower berdasarkan id_komponen
$komponenTowers = [];
...
foreach ($data as $row) {
    // Skip komponen yang tidak valid
    if ($row['nama_temuan'] === 'Tidak Ada' || $row['nama_solusi'] === 'Tidak Ada') {
        continue;
    }

    // Jika id_komponen belum ada di array, inisialisasi dengan array kosong
    if (!isset($komponenTowers[$row['id_komponen']])) {
        $komponenTowers[$row['id_komponen']] = [];
    }

    // Tambahkan nama tower ke dalam array untuk komponen yang sesuai
    $komponenTowers[$row['id_komponen']][] = $row['nama_tower'];
}
// Loop data untuk menambahkan ke slide
foreach ($data as $row) {
    // Skip kalau komponen udah diproses atau temuan/solusi tidak valid
    if (
        in_array($row['id_komponen'], $processed_komponen) ||
    ...
    // Cek dan gabungkan nama-nama tower untuk komponen yang sama
    if (isset($komponenTowers[$row['id_komponen']])) {
        $combinedTowerNames = implode(', ', $komponenTowers[$row['id_komponen']]);
    } else {
        $combinedTowerNames = $row['nama_tower']; // Default kalau cuma ada satu
    }
    ...
    $textTower->createTextRun('Tower: ' . $combinedTowerNames) // Gabungkan tower kalau lebih dari satu
...
// slide kelima
$slide5 = $presentation->createSlide(); // Tambah slide kedua

// Path gambar
$imagePath5 = __DIR__ . '/img/5.png'; // Pastikan gambar ada di folder img

// Cek apakah gambar ada
if (file_exists($imagePath5)) {
    // Gambar di slide ke-5
    $shape5 = $slide5->createDrawingShape();
    $shape5->setName('Full Slide Image 5')
        ->setDescription('Gambar Full Slide 5')
        ->setPath($imagePath5) // Menggunakan path gambar
        ->setWidth(960)        // Sesuaikan lebar (16:9)
        ->setHeight(540)       // Sesuaikan tinggi (16:9)
        ->setOffsetX(0)
        ->setOffsetY(0);       // Gambar di posisi awal
} else {
    echo "Gambar untuk slide 5 tidak ditemukan di: " . $imagePath5;
    exit;
}
//

// slide keenam
$slide6 = $presentation->createSlide(); // Tambah slide kedua

// Path gambar
$imagePath6 = __DIR__ . '/img/6.png'; // Pastikan gambar ada di folder img

// Cek apakah gambar ada
if (file_exists($imagePath6)) {
    // Gambar di slide ke-6
    $shape6 = $slide6->createDrawingShape();
    $shape6->setName('Full Slide Image 6')
        ->setDescription('Gambar Full Slide 6')
        ->setPath($imagePath6) // Menggunakan path gambar
        ->setWidth(960)        // Sesuaikan lebar (16:9)
        ->setHeight(540)       // Sesuaikan tinggi (16:9)
        ->setOffsetX(0)
        ->setOffsetY(0);       // Gambar di posisi awal
} else {
    echo "Gambar untuk slide 6 tidak ditemukan di: " . $imagePath6;
    exit;
}
//

// Pastikan folder 'PPTX' ada dan memiliki izin tulis
// Ambil nama gedung dan tanggal
$namaGedung = isset($row['nama_gedung']) ? $row['nama_gedung'] : 'Gedung_Tidak_Diketahui';
$namaGedung = preg_replace('/[^a-zA-Z0-9_]/', '_', $namaGedung); // Sanitasi nama gedung (hapus karakter ilegal)

$tanggalSekarang = date('Ymd'); // Ambil tanggal hari ini

// Gabungkan nama file
$namaFile = $namaGedung . '_' . $tanggalSekarang . '.pptx';

// Membuat writer dan outputkan langsung ke browser
try {
    // Membuat file PPTX dalam memori
    $oWriter = IOFactory::createWriter($presentation, 'PowerPoint2007');

    // Set headers untuk mendownload file PPTX
    header('Content-Type: application/vnd.openxmlformats-officedocument.presentationml.presentation');
    header('Content-Disposition: attachment;filename="' . $namaFile . '"');
    header('Cache-Control: max-age=0');

    // Output file langsung ke browser tanpa disimpan di server
    $oWriter->save('php://output'); // 'php://output' artinya file langsung dikirim ke browser
